...
while running:
    ...
    screen.fill((120,190,33))

    # Draw the coins next
    for coin in coin_list:
        screen.blit(coin.surf, coin.rect)

    #When health is equal to 0, the game ends
    if health <= 0:
        running = False
    ...
    score_font = pygame.font.SysFont("any_font", 75)
    health_font = pygame.font.SysFont("any_font", 75)
    score_block = score_font.render(f"Score: {score}", False, (0, 0, 0))
    health_block = health_font.render(f"Health: {health}", False, (0, 0, 0))
    screen.blit(score_block, (50, HEIGHT - 100))
    screen.blit(health_block, (50, HEIGHT - 50))
